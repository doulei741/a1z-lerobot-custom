...
export function Recording() {
  ...
  const requestBody = () => ({
    ...
    display_data: displayData, display_compressed_images: displayCompressed, fps,
    cameras: mode === 'dual'
    ...
    dataset: { repo_id: repoId, root, single_task: taskText, num_episodes: episodes, episode_time_s: episodeTime, reset_time_s: resetTime, fps, video },
  })
  ...
  return <div className="page">
    ...
    <div className="record-layout">
      ...
      <GlassCard className="record-config"><div className="card-heading"><div><span className="eyebrow">DATASET</span><h2>录制配置</h2></div></div><Segmented value={mode} onChange={(value) => { setMode(value); setConfigPath(value === 'single' ? 'a1z_lerobot/configs/record_a1z_single_realsense.yaml' : 'a1z_lerobot/configs/record_a1z_dual_realsense.yaml'); setLeftMapping(value === 'single' ? singleMapping : dualLeft); compatibility.reset() }} />
        ...
        <Field label="Task"><textarea rows={4} value={taskText} onChange={(event) => setTaskText(event.target.value)} /></Field><Field label="本次新增 Episode"><input type="number" min={1} value={episodes} onChange={(event) => setEpisodes(Number(event.target.value))} /></Field><div className="form-grid"><Field label="Episode Time"><input type="number" min={1} value={episodeTime} onChange={(event) => setEpisodeTime(Number(event.target.value))} /></Field><Field label="Reset Time"><input type="number" min={0} value={resetTime} onChange={(event) => setResetTime(Number(event.target.value))} /></Field></div>
