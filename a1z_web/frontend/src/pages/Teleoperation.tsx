...
export function Teleoperation() {
  ...
  const cameraPreview = useMutation({
    mutationFn: () => api.startCameraPreview({ cameras: selectedCameras, display_data: true, display_compressed_images: true }),
    onSuccess: (result) => setActive(result.task_id, result.task_type),
  })
  ...
  return <div className="page">
    ...
    <div className="teleop-layout">
      <GlassCard className="config-card"><div className="card-heading"><div><span className="eyebrow">CONFIGURATION</span><h2>运行前配置</h2></div><StatusDot state={task.data?.status === 'ready' ? 'healthy' : 'unknown'} label={task.data?.status ?? '未运行'} /></div>
        ...
        {cameraMode === 'configured' && <><div className="notice">已启用任务内 Rerun。相机仍只由 LeRobot 进程打开一次，画面会嵌入右侧，也可弹出到新窗口。</div><div className="switch-list"><label><input aria-label="Enable top camera" type="checkbox" checked={topCameraEnabled} onChange={(event) => setTopCameraEnabled(event.target.checked)} />Top D435</label><label><input aria-label="Enable left wrist camera" type="checkbox" checked={leftCameraEnabled} onChange={(event) => setLeftCameraEnabled(event.target.checked)} />{mode === 'dual' ? 'Left D405' : 'Wrist D405'}</label>{mode === 'dual' && <label><input aria-label="Enable right wrist camera" type="checkbox" checked={rightCameraEnabled} onChange={(event) => setRightCameraEnabled(event.target.checked)} />Right D405</label>}</div><div className="form-grid"><Field label="Top D435"><input list="teleop-camera-serials" value={topSerial} disabled={!topCameraEnabled} onChange={(event) => setTopSerial(event.target.value)} /></Field><Field label={mode === 'dual' ? 'Left D405' : 'Wrist D405'}><input list="teleop-camera-serials" value={leftCameraSerial} disabled={!leftCameraEnabled} onChange={(event) => setLeftCameraSerial(event.target.value)} /></Field>{mode === 'dual' && <Field label="Right D405"><input list="teleop-camera-serials" value={rightCameraSerial} disabled={!rightCameraEnabled} onChange={(event) => setRightCameraSerial(event.target.value)} /></Field>}<Field label="Camera Width"><input type="number" min={160} max={1920} value={cameraWidth} onChange={(event) => setCameraWidth(Number(event.target.value))} /></Field><Field label="Camera Height"><input type="number" min={120} max={1080} value={cameraHeight} onChange={(event) => setCameraHeight(Number(event.target.value))} /></Field><Field label="Camera FPS"><input type="number" min={1} max={60} value={cameraFps} onChange={(event) => setCameraFps(Number(event.target.value))} /></Field></div><datalist id="teleop-camera-serials">{(devices.data?.cameras ?? []).map((item) => <option value={item.serial} key={item.serial} />)}</datalist></>}
        ...
        <ErrorNotice error={(cameraPreview.error ?? guardedStart.error) as Error | null} /><div className="button-row"><Button variant="secondary" disabled={cameraMode !== 'configured' || cameraNames.length === 0 || cameraPreview.isPending || Boolean(taskId)} onClick={() => cameraPreview.mutate()}><Camera size={17} />仅预览相机（不启动机械臂）</Button><Button disabled={!confirmed || guardedStart.isPending || Boolean(taskId)} onClick={() => guardedStart.run(requestBody())}><Play size={17} />Start Teleoperation</Button></div>
      </GlassCard>
      <div className="runtime-column"><div className="joint-columns"><JointPanel title="Left A1Z" health={task.data?.health.can0} />{mode === 'dual' && <JointPanel title="Right A1Z" health={task.data?.health.can1} />}</div><RerunPreview task={task.data && ['teleoperation', 'camera_preview'].includes(task.data.task_type) ? task.data : undefined} cameraNames={cameraNames} /></div>
    </div>
