...
export const api = {
  ...
  startCameraPreview: (body: Record<string, unknown>) => request<TaskInfo>('/camera-preview/start', { method: 'POST', body: JSON.stringify(body) }),
  ...
  recordCompatibility: (body: Record<string, unknown>) => request<{ compatible: boolean; new_dataset: boolean; existing_episodes: number; checks: Record<string, boolean>; reason?: string; message?: string; action?: string }>('/record/compatibility', { method: 'POST', body: JSON.stringify(body) }),
  ...
  stopRecord: (id: string) => request<TaskInfo>(`/record/${id}/stop`, { method: 'POST', body: '{}' }),
